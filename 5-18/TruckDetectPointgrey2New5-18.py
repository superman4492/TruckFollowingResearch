...
from NeededFunctions518 import StackLeft, StackRight, OrganizeTop, LineFromPoints,Intersection,InArea,FindIntersectingLines,DrawLines,TemplateMatch,ResetToBeginning,ChooseClosestLeftandRight,GeoDistance
	    ...
	    #print "shape:", BigCap.shape[:2]
            ...
            if BigCap is not 0:
                ...
                if houghD is not None:  # filtering for straight lines
                    ...
                    #####################################_______Vertical and horizontal__________###############
                                # if horizontal and extending to either side of the middle of the screen
                                if abs(y1 - y2) < self.XrangeLines and x1 < (width / 2) and x2 > (
                                ...
                                # if vertical and both x lie on one side of the screen, and are within 1/5 of the edge
                                elif abs(x1 - x2) < self.XrangeLines and (
                                    ...
                                    if (x1 < width / 2 and x2 < width / 2):  # if on the left side
                                        LeftPoints = StackLeft([x1, x2, y1, y2], LeftPoints)

                                    elif (x1 > width / 2 and x2 > width / 2):  # if on the right side
                                        RightPoints = StackRight([x1, x2, y1, y2], RightPoints)


                #################_________ Organize the left and right lines to be closest to the top line___________ ############################

                if TopPoints is not 0:
                    TopLine=TopPoints[0,:]


                (LeftPoints,RightPoints)=ChooseClosestLeftandRight(LeftPoints,RightPoints,TopLine)
		...
		cv2.line(smallfull, (int(self.WidthOfMainVideo/2*self.WindowScale), 0), (int(self.WidthOfMainVideo/2*self.WindowScale), self.HeightOfMainVideo), (0, 0, 255), 2)
                ...
                if self.UsingTemplate is True:
                    ...
                    if self.BoxFound==4:
                        ...
                        TemplateSearchCornerOfTemplateX = self.TopLeftPoints[0]
                        TemplateSearchCornerOfTemplateY = self.TopLeftPoints[1]
                        self.BoxFound=5
                    else:
                        TemplateSearchCornerOfTemplateX=self.CornerOfTemplateX
                        TemplateSearchCornerOfTemplateY=self.CornerOfTemplateY


                    (FoundGoodMatch, self.CornerOfTemplateX, self.CornerOfTemplateY, TemplateWidth, TemplateHeight,
                     TrackMissCounter) = TemplateMatch(SmallGrey, self.CurrentTemplate, TemplateSearchCornerOfTemplateX,
                                                       TemplateSearchCornerOfTemplateY, TrackMissCounter,self.ExpandSearchFlag)
                ...
                if self.UsingTemplate is False and self.InitialMissCounter > 40:
                    (self.CenterOfTemplateX, self.CenterOfTemplateY, CompleteMissCounter, self.Old, New,  self.BoxFound, self.UsingTemplate,
                     CurrentTemplate, self.InitialMissCounter) = ResetToBeginning(self)
